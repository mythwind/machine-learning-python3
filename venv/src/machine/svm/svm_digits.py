...
class OptStruct :
    def __init__(self, data_mat, class_labels, C, toler, ktup):
        ...
        for i in range(self.m) :
            self.K[:,i] = kernel_transfer(self.X,self.X[i,:],ktup)
...
def calc_Ek_kernel(opt_struct, k) :
    fxk = float(np.multiply(opt_struct.alphas, opt_struct.label_mat).T * opt_struct.K[:, k] + opt_struct.b)
    Ek = fxk - float(opt_struct.labelMat[k])
    return Ek
...
def load_imges(dirname) :
    labels_arr = []
    file_list = os.listdir(dirname)
    m = len(file_list)
    training_mat = np.zeros((m,1024))
    for i in range(m) :    # 读取文件的前32行
        ...
        if class_num == 9 :
            ...
            labels_arr.append(1)
        training_mat[i,:] = img2vector('%s/%s' % (dirname, filename_str))
    return training_mat, labels_arr
...
def smo_p(data_mat,class_labels,C,toler,max_iter,k_tup=('lin',0)) :
    opt_str = OptStruct(np.mat(data_mat),np.mat(class_labels).transpose(),C,toler,k_tup)
    print(opt_str.__str__(True))
    iter = 0
    entry_set = True
